...
public class UnOriginAdapter extends RecyclerView.Adapter {

    private Context mContext;


    public UnOriginAdapter(Context context) {
        this.mContext = context;
    }

    List<UnOriginBean.DataBean> list = new ArrayList<>();

    List<OriginBean.DataBean.DepartmentLiseBean> lists;
    public void setDepartMember(List<OriginBean.DataBean.DepartmentLiseBean> data){
        this.lists=data;
        notifyDataSetChanged();
    }

    public void setData(List<UnOriginBean.DataBean> lists) {
        this.list = lists;
        notifyDataSetChanged();
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
        ...
        if(lists==null){
            ...
            Glide.with(mContext).load(list.get(i).getHead()).into(viewHolder1.circleImageView);
        }else{
