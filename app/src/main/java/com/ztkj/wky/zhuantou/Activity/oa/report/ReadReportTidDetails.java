...
import com.ztkj.wky.zhuantou.bean.TidDetailsBean;
...
public class ReadReportTidDetails extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //获取token和id
        sharedPreferencesHelper = new SharedPreferencesHelper(ReadReportTidDetails.this, "anhua");
        uid = (String) sharedPreferencesHelper.getSharedPreference("uid", "");
        token = (String) sharedPreferencesHelper.getSharedPreference("token", "");
        //获取cid
        sp_create_team = new SharedPreferencesHelper(ReadReportTidDetails.this, "Create_team");
...
//                ReadReportDetailsBean readReportDetailsBean = new Gson().fromJson(response, ReadReportDetailsBean.class);
                TidDetailsBean tidDetailsBean = new Gson().fromJson(response, TidDetailsBean.class);
                Log.e(TAG, "onResponse: " + response);
                if (tidDetailsBean.getErrno().equals("200")) {
//                    ReadReportDetailsBean.DataBean data = readReportDetailsBean.getData();
                    TidDetailsBean.DataBean data = tidDetailsBean.getData();
                    ...
                    Glide.with(ReadReportTidDetails.this).load(report_head)
                            .apply(options)
                            .into(detailsReportImgHead);
                    if (!data.getTitle().equals("")) {
                        detailsLinJobContents.setVisibility(View.VISIBLE);
                        detailsReportWorkContent.setText(data.getTitle());
                    }
                    if (!data.getContent().equals("")) {
                        detailsLinPerformance.setVisibility(View.VISIBLE);
                        detailsReportPerformance.setText(data.getContent());
                    }
                    if (!data.getPlan().equals("")) {
                        detailsLinPlan.setVisibility(View.VISIBLE);
                        detailsReportPlan.setText(data.getPlan());
                    }
                    List<TidDetailsBean.DataBean.ReaderBean> reader = data.getReader();
                    for (int i = 0; i < reader.size(); i++) {
                        Arr_approver.add(reader.get(i).getHead());
                    }

                    //适配器
                    ShowAdpoverAdapter showAdpoverAdapter = new ShowAdpoverAdapter(ReadReportTidDetails.this, Arr_approver);
...
//                    GridLayoutManager gridLayoutManager = new GridLayoutManager(ReadReportTidDetails.this, 3);
                    //设置RecycleView显示的方向是水平还是垂直 GridLayout.HORIZONTAL水平  GridLayout.VERTICAL默认垂直
//                    gridLayoutManager.setOrientation(GridLayout.VERTICAL);
                    reDetailsApprover.setLayoutManager(new LinearLayoutManager(ReadReportTidDetails.this));
                    reDetailsApprover.setAdapter(showAdpoverAdapter);


                } else if (tidDetailsBean.getErrno().equals("666666")) {
                    Toast.makeText(ReadReportTidDetails.this, "您的账号已在其他手机登录，如非本人操作，请修改密码", Toast.LENGTH_LONG).show();
                    JPushInterface.deleteAlias(ReadReportTidDetails.this, Integer.parseInt(uid));
                    sharedPreferencesHelper.clear();
                    SPUtils.getInstance().clear();
                    ActivityManager.getInstance().exit();
                    intent = new Intent(ReadReportTidDetails.this, NewLoginActivity.class);
