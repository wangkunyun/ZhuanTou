...
import com.ztkj.wky.zhuantou.bean.GoOutDetailsBean;
...
public class GoOutDetials extends AppCompatActivity {
    ...
    @BindView(R.id.detials_out_notes)
    TextView detialsOutNotes;
    ...
    @BindView(R.id.detials_out_place)
    TextView detialsOutPlace;
    ...
    private String TAG = "GoOutDetials";
    private String head;
    ...
    private void request(String id) {
        OkHttpUtils.post()
            ...
            public void onResponse(String response) {
                ...
                GoOutDetailsBean goOutDetailsBean = new Gson().fromJson(response, GoOutDetailsBean.class);
                GoOutDetailsBean.DataBean data = goOutDetailsBean.getData();
                ...
                detialsOutNotes.setText(data.getOut_notes());
                detialsOutPlace.setText(data.getPlace());
                //设置图片圆角角度
                RoundedCorners roundedCorners = new RoundedCorners(96);
                RequestOptions options = RequestOptions.bitmapTransform(roundedCorners);
                Glide.with(GoOutDetials.this).load(head)
                        .apply(options).into(detailsReportImgHead);
                Glide.with(GoOutDetials.this).load(data.getApprover().getHead())
                        .apply(options).into(detailsImgAppover);
                detailsTvAppover.setText(data.getApprover().getName());

                List<GoOutDetailsBean.DataBean.CopierBean> copier = data.getCopier();
                ...
                AppoverAdapter appoverAdapter = new AppoverAdapter(GoOutDetials.this, appovername, appoverhead);
                GridLayoutManager gridLayoutManager = new GridLayoutManager(GoOutDetials.this, 5);
