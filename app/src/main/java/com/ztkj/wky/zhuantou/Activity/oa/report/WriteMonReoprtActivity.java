...
public class WriteMonReoprtActivity extends AppCompatActivity {
    ...
    private MonthIDBean monthIDBean;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_write_mon_reoprt);
        ...
        //实时保存
        sp_save = new SharedPreferencesHelper(this, "sp_save_MidReport");
    ...
    protected void onResume() {
        ...
        listApprover_head_uid = new SharedPreferencesHelper(WriteMonReoprtActivity.this, "listApprover");
...
//        sava_summary.put("uid", sp_append_uid);
        ...
        //适配器
        ShowAdpoverAdapter showAdpoverAdapter = new ShowAdpoverAdapter(WriteMonReoprtActivity.this, Arr_approver);
    ...
    private void show_picture() {
        PictureSelector.create(WriteMonReoprtActivity.this)
    ...
    private void show_files() {
        Intent intentfiles = new Intent(WriteMonReoprtActivity.this, NormalFilePickActivity.class);
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                ...
                case PictureConfig.CHOOSE_REQUEST:
                    ...
                    ShowPictureAdapter showPictureAdapter = new ShowPictureAdapter(WriteMonReoprtActivity.this, listPictureName);
                ...
                case Constant.REQUEST_CODE_PICK_FILE:
                    ...
                    //适配器
                    ShowFilesAdapter showFilesAdapter = new ShowFilesAdapter(WriteMonReoprtActivity.this, ListPathName);
                    showFilesAdapter.notifyDataSetChanged();
                    reWriteReportFiles.setLayoutManager(new LinearLayoutManager(WriteMonReoprtActivity.this));
                    reWriteReportFiles.setAdapter(showFilesAdapter);
                    updateFiles(monthIDBean);
                    break;
            }
        }
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.write_report_approver:
                intent = new Intent(WriteMonReoprtActivity.this, ChooseAdressActivity.class);
    ...
    private void popuinit(String s, String s1, final String s2) {
        View contentView = LayoutInflater.from(WriteMonReoprtActivity.this).inflate(R.layout.pp_telephone, null);
        ...
        View rootview = LayoutInflater.from(WriteMonReoprtActivity.this).inflate(R.layout.activity_sz, null);
    ...
    private void request() {
        ...
        OkHttpUtils.post()
            ...
            public void onResponse(String response) {
                ...
                monthIDBean = new Gson().fromJson(response, MonthIDBean.class);
                if (monthIDBean.getErrno().equals("200")) {
...
//                    }
                ...
                } else if (monthIDBean.getErrno().equals("666666")) {
...
//                            getActivity().finish();
    ...
    private void updateFiles(MonthIDBean monthIDBean) {
        //循环上传文件
        for (int i = 0; i < list.size(); i++) {
            String path = list.get(i).getPath();
            File file = new File(path);
            OkHttpUtils.post()
                    .url(Contents.UPDATEMONTHFILE)
                    .addParams("token", token)
                    .addParams("mid", monthIDBean.getData().getMid())
    ...
    private void updatePicture(MonthIDBean monthIDBean) {
        //循环上传图片
        for (int i = 0; i < selectList.size(); i++) {
            String path = selectList.get(i).getPath();
            File file = new File(path);
            OkHttpUtils.post()
                    .url(Contents.UPDATEMONTHPIC)
                    .addParams("token", token)
                    .addParams("did", monthIDBean.getData().getMid())
    ...
    private void updatereport(MonthIDBean monthIDBean) {
        ...
        if (etWriteReportFinishWork.getText().toString().equals("") && etWriteReportworkSummary.getText().toString().equals("") && etWriteWriteWorkPlan.getText().toString().equals("")) {
            Toast.makeText(WriteMonReoprtActivity.this, "请至少输入一项", Toast.LENGTH_SHORT).show();
        } else {
            OkHttpUtils.post()
                    .url(Contents.MONTHMESSAGE)
                    .addParams("token", token)
                    .addParams("uid", uid)
                    .addParams("mid", monthIDBean.getData().getMid())
                    .addParams("weekly_completion", etWriteReportFinishWork.getText().toString())
                    .addParams("monthly_summary", etWriteReportworkSummary.getText().toString())
                    .addParams("monthly_plan", etWriteReportFinishWork.getText().toString())
    ...
    protected void onDestroy() {
        ...
        if (clickNum == 0) {
            ...
            OkHttpUtils.post()
                    ...
                    .addParams("token", token)
                    .addParams("type", "mid")
                    .addParams("id", monthIDBean.getData().getMid())
                    .build().execute(new StringCallback() {
                @Override
                public void onError(Request request, Exception e) {
