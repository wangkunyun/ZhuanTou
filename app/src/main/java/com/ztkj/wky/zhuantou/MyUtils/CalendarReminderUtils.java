...
public class CalendarReminderUtils {
    ...
    private static int checkAndAddCalendarAccount(Context context) {
        ...
        if (oldId >= 0) {
            ...
            if (addId >= 0) {
                ...
                Toast.makeText(context, "获取账号ID失败", Toast.LENGTH_SHORT).show();
                return -1;
            }
        }
    }
    ...
    private static int checkCalendarAccount(Context context) {
        ...
        try {
            if (userCursor == null) { //查询返回空值
                Toast.makeText(context, "账户ID为空", Toast.LENGTH_SHORT).show();
                return -1;
            }
    ...
    private static long addCalendarAccount(Context context) {
        ...
        Toast.makeText(context, "创建账户ID=" + id, Toast.LENGTH_SHORT).show();
        return id;
    }
     ...
     * 添加日历事件
     */
    public static void addCalendarEvent(Context context, String title, String description, long reminderTime, long reminderEndTime, int previousDate) {
        if (context == null) {
            return;
        }
        int calId = checkAndAddCalendarAccount(context); //获取日历账户的id
        if (calId < 0) { //获取账户id失败直接返回，添加日历事件失败
            Toast.makeText(context, "请先添加日历用户ID", Toast.LENGTH_SHORT).show();
            return;
        }

        //添加日历事件
        Calendar mCalendar = Calendar.getInstance();
        mCalendar.setTimeInMillis(reminderTime * 1000);//设置开始时间
        ...
        event.put(CalendarContract.Events.DTEND, reminderEndTime);
        ...
        values.put(CalendarContract.Reminders.MINUTES, previousDate * 60);// 提前previousDate天有提醒
        values.put(CalendarContract.Reminders.METHOD, CalendarContract.Reminders.METHOD_ALERT);
        Uri uri = context.getContentResolver().insert(Uri.parse(CALENDER_REMINDER_URL), values);
        Toast.makeText(context, "添加日历成功", Toast.LENGTH_SHORT).show();
        if (uri == null) { //添加事件提醒失败直接返回
            Toast.makeText(context, "添加日历时间失败", Toast.LENGTH_SHORT).show();
            return;
        }
    }
