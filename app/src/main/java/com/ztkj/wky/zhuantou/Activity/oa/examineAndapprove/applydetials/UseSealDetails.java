...
import com.ztkj.wky.zhuantou.adapter.UserSealDetailsBean;
...
public class UseSealDetails extends AppCompatActivity {
    ...
    @BindView(R.id.detials_seal_type)
    TextView detialsSealType;
    @BindView(R.id.detials_seal_date)
    TextView detialsSealDate;
    @BindView(R.id.detials_files_name)
    TextView detialsFilesName;
    @BindView(R.id.detials_files_num)
    TextView detialsFilesNum;
    ...
    private String TAG = "UseSealDetails";
    private String head;
    private List<UserSealDetailsBean.DataBean.CopierBean> copier;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_use_seal_details);
        ButterKnife.bind(this);

        toolbarTitle.setText("用印申请");
        intent = getIntent();
    ...
    private void request(String id) {
        OkHttpUtils.post()
            ...
            public void onResponse(String response) {
                ...
                UserSealDetailsBean userSealDetailsBean = new Gson().fromJson(response, UserSealDetailsBean.class);
                UserSealDetailsBean.DataBean data = userSealDetailsBean.getData();
                detailsReportName.setText(data.getU_name() + "的用印申请");
                detailsReportTime.setText(data.getAddtime());
                detialsFilesName.setText(data.getFile_name());
                detialsSealType.setText(data.getSeal_type());
                detialsFilesNum.setText(data.getFile_number());
                ...
                long l = Long.parseLong(data.getPir_time() + "000");

                SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd-HH");
                detialsSealDate.setText(simpleDateFormat.format(new Date(l)));
                //设置图片圆角角度
                RoundedCorners roundedCorners = new RoundedCorners(96);
                RequestOptions options = RequestOptions.bitmapTransform(roundedCorners);
                Glide.with(UseSealDetails.this).load(head)
                        .apply(options).into(detailsReportImgHead);
                Glide.with(UseSealDetails.this).load(data.getApprover().getHead())
                        .apply(options).into(detailsImgAppover);
                detailsTvAppover.setText(data.getApprover().getName());

                copier = data.getCopier();
                ...
                AppoverAdapter appoverAdapter = new AppoverAdapter(UseSealDetails.this, appovername, appoverhead);
                GridLayoutManager gridLayoutManager = new GridLayoutManager(UseSealDetails.this, 5);
