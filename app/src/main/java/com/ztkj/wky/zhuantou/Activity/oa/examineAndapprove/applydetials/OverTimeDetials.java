...
import com.ztkj.wky.zhuantou.bean.OverTimeDetialsBean;
...
public class OverTimeDetials extends AppCompatActivity {
    ...
    private String TAG = "OverTimeDetials";
    private String head;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_over_time_detials);
        ButterKnife.bind(this);

        toolbarTitle.setText("加班申请");
        intent = getIntent();
    ...
    private void request(String id) {
        OkHttpUtils.post()
            ...
            public void onResponse(String response) {
                Log.e(TAG, "onResponse: 0" + response);
                OverTimeDetialsBean overTimeDetialsBean = new Gson().fromJson(response, OverTimeDetialsBean.class);
                OverTimeDetialsBean.DataBean data = overTimeDetialsBean.getData();
                ...
                detialsObjective.setText(data.getOver_note());

                //设置图片圆角角度
                RoundedCorners roundedCorners = new RoundedCorners(96);
                RequestOptions options = RequestOptions.bitmapTransform(roundedCorners);
                Glide.with(OverTimeDetials.this).load(head)
                        .apply(options).into(detailsReportImgHead);
                Glide.with(OverTimeDetials.this).load(data.getApprover().getHead())
                        .apply(options).into(detailsImgAppover);
                detailsTvAppover.setText(data.getApprover().getName());

                List<OverTimeDetialsBean.DataBean.CopierBean> copier = data.getCopier();
                ...
                AppoverAdapter appoverAdapter = new AppoverAdapter(OverTimeDetials.this, appovername, appoverhead);
                GridLayoutManager gridLayoutManager = new GridLayoutManager(OverTimeDetials.this, 5);
